defmodule BackendWeb.CoreComponents do
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "pointer-events-auto w-full rounded-2xl border bg-white/95 px-4 py-3 shadow-lg backdrop-blur",
        @kind == :info && "border-emerald-200 text-emerald-900",
        @kind == :error && "border-rose-200 text-rose-900"
      ]}
      {@rest}
    >
      <div class="flex items-start gap-3">
        ...
        <button type="button" class="group -mt-1" aria-label={gettext("close")}>
          <.icon name="hero-x-mark" class="size-5 opacity-40 transition group-hover:opacity-70" />
        </button>
      </div>
    </div>
    """
  ...
  def button(%{rest: rest} = assigns) do
    variants = %{
      ...
      nil => "border border-slate-200 bg-white text-slate-700 hover:border-slate-300"
    }

    base_classes =
      "inline-flex h-11 items-center justify-center gap-2 rounded-full px-5 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 disabled:cursor-not-allowed disabled:opacity-60"
    ...
    <div class="mb-2">
      <label class="flex items-center gap-2 text-sm text-slate-700">
        ...
        <input
          ...
          class={
            @class ||
              "h-4 w-4 rounded border-slate-300 text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-200"
          }
          {@rest}
        />
        <span>{@label}</span>
  ...
  defp error(assigns) do
    ~H"""
    <p class="mt-1.5 flex items-center gap-2 text-xs font-medium text-rose-600">
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", "pb-4"]}>
      <div>
        <h1 class="text-lg font-semibold leading-8 text-slate-900">
          {render_slot(@inner_block)}
        </h1>
  ...
  def table(assigns) do
    ...
    <table class="w-full text-left text-sm text-slate-700">
      ...
      >
        <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="hover:bg-slate-50">
          <td
            :for={col <- @col}
          ...
          <td :if={@action != []} class="px-4 py-3 font-semibold">
  ...
  def list(assigns) do
    ...
    <ul class="divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white">
      <li :for={item <- @item} class="px-4 py-4">
        <div class="text-sm font-semibold text-slate-900">{item.title}</div>
        <div class="text-sm text-slate-600">{render_slot(item)}</div>
      </li>
    </ul>
    """
  end
